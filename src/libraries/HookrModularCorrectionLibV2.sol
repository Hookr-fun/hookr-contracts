...
/// @notice Fail-open dispatcher for phase-bound modular WTH correction plans.
library HookrModularCorrectionLibV2 {
    uint256 internal constant SIGNED_HOOK_DATA_LENGTH = 480;
    /// @dev Route lookup is one mapping read by signed routeId; the number of registered pools is
    ///      never iterated. The larger bound leaves measured headroom for both v3 callback
    ///      settlement and v4 nested-hook execution while keeping outer settlement gas reserved.
    uint256 internal constant EXECUTOR_GAS_STIPEND = 2_500_000;
    ...
    bytes32 internal constant SKIP_INELIGIBLE = keccak256("HOOKR_CORRECTION_V2_INELIGIBLE");
    bytes32 internal constant SKIP_BAD_PAYLOAD = keccak256("HOOKR_CORRECTION_V2_BAD_PAYLOAD");
    bytes32 internal constant SKIP_PHASE = keccak256("HOOKR_CORRECTION_V2_PHASE_MISMATCH");
    bytes32 internal constant SKIP_EXPIRED = keccak256("HOOKR_CORRECTION_V2_EXPIRED");
    bytes32 internal constant SKIP_CLOCK = keccak256("HOOKR_CORRECTION_V2_CLOCK_UNAVAILABLE");
    bytes32 internal constant SKIP_LOW_GAS = keccak256("HOOKR_CORRECTION_V2_LOW_GAS");
    ...
    function dispatch(
        ...
        if (
            executor == address(0) || triggerBaseAmount == 0 || authenticatedRecipient == address(0)
                || creator == address(0) || raw.length == 0
        ) {
            result.reasonHash = SKIP_INELIGIBLE;
            return result;
        }
        if (gasleft() <= EXECUTOR_GAS_STIPEND + DISPATCH_GAS_RESERVE) {
            result.reasonHash = SKIP_LOW_GAS;
            return result;
        }
        ...
        HookrArbTypesV3.ExecutionRequest memory request = HookrArbTypesV3.ExecutionRequest({
            ...
            profitSplit: HookrArbTypesV3.ProfitSplit({
                creator: creator,
                traderBps: HookrWthFeePolicyV2.TRADER_BPS,
                creatorBps: HookrWthFeePolicyV2.CREATOR_BPS,
                triggerPoolBps: HookrWthFeePolicyV2.TRIGGER_POOL_LP_BPS
            }),
