...
contract HookrLockRewardsV3Test is Test {
    ...
    function setUp() public {
        ...
        boost = new HookrLaunchBoostV3(hookr, _launchpadSet(address(launchpad)), IHookrLockRewardsV2(address(rewards)));
    ...
    /// @dev V3 takes the launchpad set as an array; every ported case runs the one-generation set,
    ///      which is exactly the V2 shape. Multi-generation behaviour lives in HookrLaunchBoostV3.t.sol.
    function _launchpadSet(address pad) internal pure returns (HookrLaunchpad[] memory set) {
    ...
    function test_bootstrapScheduleIsTwoHoursThenPositiveMondayBridge() public view {
        ...
        assertEq(boost.launchpadCount(), 1);
        assertEq(address(boost.launchpad0()), address(launchpad));
    }
    ...
    function test_exactMondayClaimBoundaryStillCreatesPositiveBridge() public {
        ...
        // Deploy exactly two bootstrap epochs before the boundary so claimEpochStartsAt lands on
        // it precisely. Derived from the constant, not hardcoded, so shortening the bootstrap
        // cannot silently stop exercising this edge.
        vm.warp(monday - 2 * uint256(rewards.BOOTSTRAP_EPOCH_LENGTH()));
        MockHookrUtilityToken localToken = new MockHookrUtilityToken();
        HookrLockRewardsV3 localRewards = new HookrLockRewardsV3(localToken, address(this));
    ...
    function test_preOpenBoostAndPermitPathRevertAtomically() public {
        ...
        vm.prank(creator);
        vm.expectRevert(abi.encodeWithSelector(HookrLaunchBoostV3.BoostingNotOpen.selector, rewards.feeEpochStartsAt()));
    ...
    function test_epochZeroLockFeeBearingEpochOneAndClaimEpochTwoSameDay() public {
        ...
        HookrLockRewardsV3.Position memory p = rewards.position(positionId);
        ...
        vm.prank(alice);
        vm.expectRevert(HookrLockRewardsV3.NothingToClaim.selector);
        ...
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(HookrLockRewardsV3.PositionStillLocked.selector, p.unlockAt));
    ...
    function test_tierDurationsWeightsAndPrincipalCustodyRemainExact() public {
        ...
        HookrLockRewardsV3.Position memory p30 = rewards.position(id30);
        HookrLockRewardsV3.Position memory p90 = rewards.position(id90);
        HookrLockRewardsV3.Position memory p180 = rewards.position(id180);
        ...
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(HookrLockRewardsV3.PositionStillLocked.selector, p30.unlockAt));
    ...
    function test_boundedCheckpointStillRequiresPermissionlessCatchup() public {
        ...
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(HookrLockRewardsV3.CheckpointRequired.selector, 64, target));
    ...
    function test_rewardSourceIsOneShotAndNonSourceCannotNotify() public {
        MockHookrUtilityToken localToken = new MockHookrUtilityToken();
        HookrLockRewardsV3 fresh = new HookrLockRewardsV3(localToken, address(this));
        fresh.setRewardSourceOnce(address(boost));
        assertEq(fresh.rewardSource(), address(boost));
        assertEq(fresh.configurator(), address(0));
        vm.expectRevert(HookrLockRewardsV3.NotConfigurator.selector);
        fresh.setRewardSourceOnce(address(boost));
        vm.prank(outsider);
        vm.expectRevert(HookrLockRewardsV3.NotRewardSource.selector);
    ...
    function test_capacityFailureCannotMutateRewardOrCustodyState() public {
        ...
        vm.prank(creator);
        vm.expectRevert(abi.encodeWithSelector(HookrLaunchBoostV3.CapacityFull.selector, 512));
    ...
    function test_prunePreservesCustodyAndCreatorCanWithdraw() public {
        ...
        HookrLaunchBoostV3.BoostPosition memory beforePosition = boost.boostOf(launchToken);
        vm.warp(beforePosition.unlockAt);
        vm.prank(outsider);
        boost.pruneExpired(launchToken);
        HookrLaunchBoostV3.BoostPosition memory afterPrune = boost.boostOf(launchToken);
    ...
    function test_hostileTokenBehaviorAndCallbacksRemainAtomic() public {
        ...
        HookrLockRewardsV3 localRewards = new HookrLockRewardsV3(token, address(this));
        HookrLaunchBoostV3 localBoost =
            new HookrLaunchBoostV3(token, _launchpadSet(address(pad)), IHookrLockRewardsV2(address(localRewards)));
        ...
        token.setCallback(
            ...
            abi.encodeCall(
                HookrLockRewardsV3.lock, (100 ether, 0, type(uint32).max, type(uint40).max, block.timestamp + 1 hours)
            )
        );
        vm.prank(alice);
        localRewards.lock(1_000 ether, 0, type(uint32).max, type(uint40).max, block.timestamp + 1 hours);
        ...
        assertEq(token.callbackRevertSelector(), HookrLockRewardsV3.ReentrantCall.selector);
        ...
        vm.prank(creator);
        vm.expectRevert(HookrLockRewardsV3.AccountingInvariant.selector);
