...
/// @title Hookr Lock Rewards V1
/// @notice Non-transferable, fixed-term $HOOKR locks that receive realized Launch Boost fees.
/// @dev Principal is never slashed and carries no entry fee. Positions activate only after the
///      next Monday 00:00 UTC boundary and earn only for complete weekly epochs. The sole reward
///      source is wired once after deployment; there is no owner, pause, upgrade or sweep path.
contract HookrLockRewardsV1 {
    ...
    function currentEpoch() public view returns (uint32 epoch) {
        uint256 raw = (block.timestamp - MONDAY_EPOCH_OFFSET) / EPOCH_LENGTH;
    ...
    function epochStart(uint32 epoch) public pure returns (uint40 timestamp) {
        uint256 raw = uint256(MONDAY_EPOCH_OFFSET) + uint256(epoch) * EPOCH_LENGTH;
    ...
    function epochAt(uint40 timestamp) public pure returns (uint32 epoch) {
        if (timestamp < MONDAY_EPOCH_OFFSET) revert AccountingInvariant();
        uint256 raw = (uint256(timestamp) - MONDAY_EPOCH_OFFSET) / EPOCH_LENGTH;
        if (raw > type(uint32).max) revert AccountingInvariant();
        // Safe after the explicit uint32 upper bound above.
        // forge-lint: disable-next-line(unsafe-typecast)
        epoch = uint32(raw);
    ...
    /// @notice Permissionlessly settle at most maxEpochs completed weeks.
    ...
    function checkpoint(uint32 maxEpochs) public returns (uint32 processedThroughEpoch) {
        ...
        while (cursor < target && processed < maxEpochs) {
            ...
            if (fees != 0 && eligibleWeight != 0) {
                ...
                // Dust stays inside rewardReserve and is never indexed twice. Re-feeding a whole
                // token remainder into every later index can accumulate fractional index deltas
                // until positions claim more than the reserve. V1 deliberately records the dust
                // instead; there is no owner sweep, so it remains permanently lossless backing.
