...
/// @title HookrAttachHook
/// @notice The attach-generation HookrHook: one shared Uniswap v4 hook serving every pool opened
///         through `HookrExistingTokenFactory` for a token that already exists. Per-pool behavior
///         is configured once by the factory at attach time and is immutable afterwards. The ONLY
///         intended difference from `HookrHook` is the authorizing identity: the single immutable
///         `launchpad` becomes the single immutable `factory` — same one-shot write-once
///         `configurePool`, same `beforeInitialize` sender gate, same guard-window LP exemption,
///         all now naming the factory. Deliberately NOT a multi-initializer ACL: one immutable
///         authority, no setter, no owner path, so the fail-closed 1:1 hook<->deployer binding of
///         the launchpad generation carries over unchanged. Keep every other line byte-equal to
///         `HookrHook` so the audit diff stays mechanical. Blocks compose per launch:
///
...
///                        factory's locked positions; permissionless LP resumes at its end.
///                        What those locked positions earn during the window is
///                        recorded (`guardLpEarnedWei`) and withheld from the creator's fee share
///                        by the factory, so the tax is not rebated to a creator who paid it.
...
contract HookrAttachHook is IHooks, IUnlockCallback {
    ...
    ///         the factory and the hook so a blueprint cannot create one-wei schedule slots.
    ...
    /// @notice Conservatively accounted ETH the factory-owned LPs earned while the anti-snipe
    ...
    /// @dev Read by `HookrExistingTokenFactory.collectPoolFees`, which withholds this much of the
    ///      collected ETH from the creator's share. Without it the snipe tax flows back to whoever
    ///      paid it whenever that party is also the creator. This is an accrual figure computed
    ///      from the swap inputs the hook sees, not a balance the hook holds — the ETH itself is
    ///      fee growth inside the PoolManager, on the factory's own position.
    ...
    /// @notice Called by the factory immediately before pool initialization. One-shot.
    ...
    ///         Called by the factory right after initialization; also callable by anyone later
    ...
    function beforeInitialize(address sender, PoolKey calldata key, uint160)
        ...
        returns (bytes4)
    {
        // Only the factory may attach this hook, only with the dynamic-fee flag, and only
        ...
        returns (bytes4, BeforeSwapDelta, uint24)
    {
        ...
        if (isBuy && exactIn) {
            ...
            // LP and deterministic-pot cuts are returned as a beforeSwap specified delta.
        ...
        // it lands in fee growth the factory's locked positions later collect and split with the
        ...
        //
        // So conservatively account ETH these factory-owned positions earn while the guard is
        // live, and let `HookrExistingTokenFactory.collectPoolFees` withhold no more than that from
        // the creator side. The figure uses v4's protocol-net LP fee arithmetic on the input that
        // reaches the swap — the specified amount less the hook's own cut — plus the in-swap LP
        // donation. It is cumulative and never decremented; the factory tracks how much it already
        // withheld.
        if (guardActive && isBuy) {
    ...
    ///      the LP override as a fee on the whole input can overstate the factory's earnings and
    ...
    ///      but can never divert later creator fees for LP income the factory did not earn.
        ...
        returns (bytes4)
    {
        ...
        // `guardLpEarnedWei` is the amount the factory's locked positions must withhold from
        ...
        // phantom debt that diverts unrelated post-guard creator fees. The factory is allowed to
        ...
        if (cfg.guardEndBlock != 0 && block.number < cfg.guardEndBlock && sender != factory) {
    ...
    {
        // Exact-input native ETH -> token only. Sells and exact-output buys preserve the v2
        // boundary and do not pay an auto-burn cut.
        ...
        if (nativeCutBps > 0) {
            uint256 expectedPoolIn = requested - ((requested * nativeCutBps) / BPS);
        ...
        if (guardActive) {
            ...
            uint256 actualBuyIn = actualPoolIn + ((requested * nativeCutBps) / BPS);
