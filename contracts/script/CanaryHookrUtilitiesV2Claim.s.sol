...
contract CanaryHookrUtilitiesV2Claim is HookrUtilityV2CanaryBase {
    function run() external {
        UtilityContracts memory u = _loadUtilityContracts();
        (uint256 positionId, HookrLockRewardsV2.Position memory lockPosition) = _canaryPosition(u.rewards);
        ...
        HookrLaunchBoostV2.BoostPosition memory boostPosition = u.boost.boostOf(token);
        ...
        HookrLockRewardsV2.Position memory settled = u.rewards.position(positionId);
        ...
        console2.log("utility V2 canary stage 3 claimed", reward);
        console2.log("utility V2 canary post-claim reserve", u.rewards.rewardReserve());
    }
}
