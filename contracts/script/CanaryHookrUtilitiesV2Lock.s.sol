...
/// @notice Stage 1/3: exactly one 1-HOOKR approval and one tier-0 lock in bootstrap epoch zero.
/// @dev Any completed lock or partially mined approval makes the preconditions fail on rerun.
contract CanaryHookrUtilitiesV2Lock is HookrUtilityV2CanaryBase {
    function run() external {
        ...
        (uint256 derivedPositionId, HookrLockRewardsV2.Position memory p) = _canaryPosition(u.rewards);
        ...
        console2.log("utility V2 canary stage 1 position", positionId);
        console2.log("utility V2 fee epoch", p.activationEpoch);
        console2.log("utility V2 fee epoch starts", u.rewards.feeEpochStartsAt());
    }
}
