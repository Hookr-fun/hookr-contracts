...
contract CanaryHookrUtilitiesV3Boost is HookrUtilityV3CanaryBase {
    ...
    function run() external {
        ...
        (, IHookrLockRewardsV3Canary.Position memory lockPosition) = _canaryPosition(u.rewards);
        ...
        // One position per token: a nonzero principal here means this token is already boosted.
        ...
        // Unchanged from V2: the 512-slot registry must have room BEFORE the approval is broadcast,
        // so a full registry fails closed instead of stranding an allowance on a Boost that reverts.
        ...
        IHookrLaunchBoostV3Canary.BoostPosition memory p = u.boost.boostOf(token);
        ...
        console2.log("utility V3 canary stage 2 token", token);
        console2.log("utility V3 canary reward fee", fee);
        console2.log("utility V3 claim epoch starts", u.rewards.claimEpochStartsAt());
    }
}
