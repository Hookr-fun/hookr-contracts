...
import {
  UTILITY_V2_CANARY_SPEC,
  UTILITY_V2_RECEIPT_LABELS,
  validateUtilityV2CanaryEvidence,
} from "./lib/utility-v2-canary-evidence.mjs";
...
const DEPLOY_PATH = flag("deploy", "contracts/broadcast/DeployHookrUtilitiesV2.s.sol/4663/run-latest.json");
const LOCK_PATH = flag("lock", "contracts/broadcast/CanaryHookrUtilitiesV2Lock.s.sol/4663/run-latest.json");
const BOOST_PATH = flag("boost", "contracts/broadcast/CanaryHookrUtilitiesV2Boost.s.sol/4663/run-latest.json");
const CLAIM_PATH = flag("claim", "contracts/broadcast/CanaryHookrUtilitiesV2Claim.s.sol/4663/run-latest.json");
...
const fail = (message) => {
  console.error(`\nUTILITY V2 CANARY EVIDENCE BLOCKED: ${message}`);
  process.exit(1);
};
...
const relevantPaths = [
  ...
  "contracts/script/HookrUtilityV2CanaryBase.s.sol",
  "contracts/script/CanaryHookrUtilitiesV2Lock.s.sol",
  "contracts/script/CanaryHookrUtilitiesV2Boost.s.sol",
  "contracts/script/CanaryHookrUtilitiesV2Claim.s.sol",
  ...
  "scripts/lib/utility-v2-canary-evidence.mjs",
...
const expectedDeployShape = [
  ["CREATE", "HookrLockRewardsV2"],
  ["CREATE", "HookrLaunchBoostV2"],
];
...
const expectedRewardsInit = encodeDeployData({
  ...
  args: [UTILITY_V2_CANARY_SPEC.hookrToken, UTILITY_V2_CANARY_SPEC.deployer],
});
const expectedBoostInit = encodeDeployData({
  ...
  args: [UTILITY_V2_CANARY_SPEC.hookrToken, coreLaunchpad, lockRewards],
});
...
for (let index = 0; index < allPairs.length; index += 1) {
  const label = UTILITY_V2_RECEIPT_LABELS[index];
  ...
  if (
    getAddress(receipt.from) !== UTILITY_V2_CANARY_SPEC.deployer ||
    getAddress(transaction.from) !== UTILITY_V2_CANARY_SPEC.deployer
  ) {
...
const expectedTargets = [
  lockRewards,
  UTILITY_V2_CANARY_SPEC.hookrToken,
  lockRewards,
  UTILITY_V2_CANARY_SPEC.hookrToken,
  launchBoost,
  lockRewards,
...
const approveLockEvent = exactlyOne(onlyEvents(records[3], UTILITY_V2_CANARY_SPEC.hookrToken, tokenAbi, "Approval"), "lock approval");
const lockArgs = decodeCall(records[4], rewardsAbi, "lock");
const lockTransfers = onlyEvents(records[4], UTILITY_V2_CANARY_SPEC.hookrToken, tokenAbi, "Transfer");
...
const approveBoostEvent = exactlyOne(onlyEvents(records[5], UTILITY_V2_CANARY_SPEC.hookrToken, tokenAbi, "Approval"), "boost approval");
const boostArgs = decodeCall(records[6], boostAbi, "boostByIntent");
const boostTransfers = onlyEvents(records[6], UTILITY_V2_CANARY_SPEC.hookrToken, tokenAbi, "Transfer");
...
const claimTransfer = exactlyOne(onlyEvents(records[7], UTILITY_V2_CANARY_SPEC.hookrToken, tokenAbi, "Transfer"), "claim transfer");
...
const [hookrCode, rewardsCode, boostCode, launchpadCode] = await Promise.all([
  runtime(UTILITY_V2_CANARY_SPEC.hookrToken, "HOOKR token"),
...
if (!sameHex(runtimes.hookrToken, UTILITY_V2_CANARY_SPEC.hookrRuntimeCodehash)) fail("HOOKR runtime is not reviewed");
...
const [
  ...
  read(lockRewards, rewardsAbi, "contractName"),
  read(lockRewards, rewardsAbi, "contractVersion"),
  read(launchBoost, boostAbi, "contractName"),
  read(launchBoost, boostAbi, "contractVersion"),
  read(lockRewards, rewardsAbi, "BOOTSTRAP_EPOCH_LENGTH"),
  read(lockRewards, rewardsAbi, "bootstrapStartedAt"),
  read(lockRewards, rewardsAbi, "feeEpochStartsAt"),
  read(lockRewards, rewardsAbi, "claimEpochStartsAt"),
  read(lockRewards, rewardsAbi, "weeklyEpochsStartAt"),
  read(launchBoost, boostAbi, "boostingOpensAt"),
  read(launchBoost, boostAbi, "boostingOpen"),
]);
if (rewardsName !== "HookrLockRewards" || rewardsVersion !== "2") fail("LockRewards V2 identity is wrong");
if (boostName !== "HookrLaunchBoost" || boostVersion !== "2") fail("LaunchBoost V2 identity is wrong");
...
if (bootstrapStart !== records[0].timestamp) fail("bootstrap start does not equal the Rewards deployment block timestamp");
...
if (BigInt(boostingOpensAt) !== feeStart) fail("LaunchBoost opening does not equal the fee epoch start");
...
try {
  validateReviewedUtilityV2Runtime({
    ...
    expectedImmutableWords: [
      utilityV2AddressWord(UTILITY_V2_CANARY_SPEC.hookrToken),
  ...
  validateReviewedUtilityV2Runtime({
    ...
    expectedImmutableWords: [
      utilityV2AddressWord(UTILITY_V2_CANARY_SPEC.hookrToken),
      utilityV2AddressWord(coreLaunchpad),
      utilityV2AddressWord(lockRewards),
      utilityV2UintWord(feeStart),
    ],
...
if (getAddress(launch.creator ?? launch[1]) !== UTILITY_V2_CANARY_SPEC.deployer) fail("canonical launch creator is wrong");

const lockedPositionId = BigInt(lockedEvent.positionId);
if (lockedPositionId <= 0n) fail("lock receipt has a zero position id");
const ownerPositionIds = await read(lockRewards, rewardsAbi, "positionsOf", [UTILITY_V2_CANARY_SPEC.deployer, 0n, 2n]);
...
if (getAddress(await read(lockRewards, rewardsAbi, "hookrToken")) !== UTILITY_V2_CANARY_SPEC.hookrToken) fail("LockRewards token link is wrong");
...
if (getAddress(await read(launchBoost, boostAbi, "hookrToken")) !== UTILITY_V2_CANARY_SPEC.hookrToken) fail("LaunchBoost token link is wrong");
...
if (await read(launchBoost, boostAbi, "MIN_GROSS_BOOST") !== UTILITY_V2_CANARY_SPEC.minGrossBoost) {
  fail("LaunchBoost minimum gross policy is wrong");
}
if (await read(launchBoost, boostAbi, "MAX_ACTIVE_BOOSTS") !== UTILITY_V2_CANARY_SPEC.maxActiveBoosts) {
...
const evidence = {
  ...
  bootstrap: {
    bootstrapStartedAt: asSafeNumber(bootstrapStart, "bootstrap start"),
    feeEpochStartsAt: asSafeNumber(feeStart, "fee epoch start"),
    claimEpochStartsAt: asSafeNumber(claimStart, "claim epoch start"),
    weeklyEpochsStartAt: asSafeNumber(weeklyStart, "weekly epoch start"),
  },
  core: {
  ...
  identities: {
    deployer: UTILITY_V2_CANARY_SPEC.deployer,
    hookrToken: UTILITY_V2_CANARY_SPEC.hookrToken,
    lockRewards,
    launchBoost,
  ...
  canonicalLaunch: {
    intentId: UTILITY_V2_CANARY_SPEC.coreIntentId,
    token: launchToken,
  ...
  postconditions: {
    ...
    allowances: {
      lockRewards: await read(UTILITY_V2_CANARY_SPEC.hookrToken, tokenAbi, "allowance", [
        UTILITY_V2_CANARY_SPEC.deployer,
        lockRewards,
      ]),
      launchBoost: await read(UTILITY_V2_CANARY_SPEC.hookrToken, tokenAbi, "allowance", [
        UTILITY_V2_CANARY_SPEC.deployer,
        launchBoost,
      ]),
    },
    lockAccounting: {
      ...
      ownerPositionCount: await read(lockRewards, rewardsAbi, "positionsOfCount", [UTILITY_V2_CANARY_SPEC.deployer]),
...
try {
  validateUtilityV2CanaryEvidence(evidence);
} catch (error) {
...
if (JSON_OUTPUT) {
  ...
  console.log("UTILITY V2 CANARY EVIDENCE OK (read-only)");
