...
import {
  UTILITY_V2_CANARY_SPEC as spec,
  UTILITY_V2_RECEIPT_LABELS,
  UTILITY_V2_REWARD_Q128,
  utilityV2EpochAt,
  utilityV2ProRataReward,
  validateUtilityV2BootstrapSchedule,
...
const bootstrapStartedAt = 1_800_000_000;
const feeEpochStartsAt = bootstrapStartedAt + spec.bootstrapEpochSeconds;
const claimEpochStartsAt = feeEpochStartsAt + spec.bootstrapEpochSeconds;
const currentMonday = spec.mondayOffsetSeconds +
  Math.floor((claimEpochStartsAt - spec.mondayOffsetSeconds) / spec.weeklyEpochSeconds) * spec.weeklyEpochSeconds;
const weeklyEpochsStartAt = currentMonday > claimEpochStartsAt
  ? currentMonday
  : currentMonday + spec.weeklyEpochSeconds;
...
function fixture() {
  ...
  const claimStartIndex = 3n * UTILITY_V2_REWARD_Q128;
  const claimEndIndex = claimStartIndex + UTILITY_V2_REWARD_Q128 / 4n;
  const claimedReward = utilityV2ProRataReward(spec.lockGross, claimStartIndex, claimEndIndex);
  const depositTimestamp = bootstrapStartedAt + 60;
  const depositEpoch = utilityV2EpochAt(bootstrap, depositTimestamp);
  const boostTimestamp = feeEpochStartsAt + 60;
  const claimTimestamp = claimEpochStartsAt + 60;
  const receiptOrder = UTILITY_V2_RECEIPT_LABELS.map((label, index) => ({
    label,
    hash: hash(index + 1),
    ...
    stages: {
      ...
      lock: {
        ...
        event: {
          ...
          expiryEpoch: utilityV2EpochAt(bootstrap, depositTimestamp + spec.lockDurationSeconds),
...
test("accepts exact canary receipts and pro-rata proof amid unrelated ambient state", () => {
  assert.equal(validateUtilityV2CanaryEvidence(fixture()).chainId, 4663);
});

test("maps exact V2 schedule boundaries to local epochs 0, 1, 2, and weekly 3+", () => {
  assert.deepEqual(validateUtilityV2BootstrapSchedule(bootstrap), bootstrap);
  assert.equal(utilityV2EpochAt(bootstrap, bootstrapStartedAt), 0);
  assert.equal(utilityV2EpochAt(bootstrap, feeEpochStartsAt - 1), 0);
  assert.equal(utilityV2EpochAt(bootstrap, feeEpochStartsAt), 1);
  assert.equal(utilityV2EpochAt(bootstrap, claimEpochStartsAt), 2);
  assert.equal(utilityV2EpochAt(bootstrap, weeklyEpochsStartAt), 3);
  assert.equal(utilityV2EpochAt(bootstrap, weeklyEpochsStartAt + spec.weeklyEpochSeconds), 4);
});

test("rejects schedule drift and a pre-bootstrap timestamp", () => {
  assert.throws(
    () => validateUtilityV2BootstrapSchedule({ ...bootstrap, feeEpochStartsAt: feeEpochStartsAt + 1 }),
    /fee epoch start/,
  );
  assert.throws(
    () => validateUtilityV2BootstrapSchedule({ ...bootstrap, weeklyEpochsStartAt: weeklyEpochsStartAt + spec.weeklyEpochSeconds }),
    /first Monday strictly after/,
  );
  assert.throws(() => utilityV2EpochAt(bootstrap, bootstrapStartedAt - 1), /predates bootstrap/);
});

test("rejects a non-exact 1% fee transfer", () => {
  const evidence = fixture();
  evidence.stages.boost.feeTransfer.amount -= 1n;
  assert.throws(() => validateUtilityV2CanaryEvidence(evidence), /boost fee transfer amount is wrong/);
});

test("rejects a claim for any position other than the lock receipt position", () => {
  const evidence = fixture();
  evidence.stages.claim.calldata.positionId += 1n;
  assert.throws(() => validateUtilityV2CanaryEvidence(evidence), /claim position id is wrong/);
});
...
test("rejects a claim amount not proven by the immutable epoch-index delta", () => {
  ...
  assert.throws(() => validateUtilityV2CanaryEvidence(evidence), /pro-rata claimed reward is wrong/);
});
...
test("rejects ambient ledgers below the canary's receipt-scoped contribution", () => {
  ...
  assert.throws(() => validateUtilityV2CanaryEvidence(evidence), /fees forwarded ledger is below the canary proof/);
});

test("rejects legacy one-HOOKR Boost canary evidence", () => {
  const evidence = fixture();
  evidence.stages.boost.calldata.grossAmount = spec.lockGross;
  assert.throws(() => validateUtilityV2CanaryEvidence(evidence), /boost calldata gross is wrong/);
});
...
test("rejects a changed active-capacity policy or missing canonical membership", () => {
  ...
  assert.throws(() => validateUtilityV2CanaryEvidence(wrongCapacity), /maximum active boosts is wrong/);

  const prunedCanary = fixture();
  prunedCanary.postconditions.boostAccounting.registered = false;
  assert.throws(() => validateUtilityV2CanaryEvidence(prunedCanary), /canonical boost is not registered/);
});
...
test("rejects duplicate receipts and the wrong Robinhood L1 contract clock", () => {
  ...
  assert.throws(() => validateUtilityV2CanaryEvidence(duplicate), /duplicate hash/);

  const wrongClock = fixture();
  wrongClock.postconditions.boostPosition.startedAtBlock = 106;
  assert.throws(() => validateUtilityV2CanaryEvidence(wrongClock), /boost L1 block clock is wrong/);
});
