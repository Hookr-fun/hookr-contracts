...
export const UTILITY_CANARY_SPEC = Object.freeze({
  ...
  epochSeconds: 7 * 24 * 60 * 60,
  epochOffsetSeconds: 4 * 24 * 60 * 60,
  canaryDeadlineWindowSeconds: 10 * 60,
});

export const UTILITY_REWARD_Q128 = 1n << 128n;

export const UTILITY_RECEIPT_LABELS = Object.freeze([
  "lockRewardsDeploy",
  "launchBoostDeploy",
  "rewardSourceSet",
  "approveLock",
...
export function utilityEpochAt(timestamp) {
  const value = asBigInt(timestamp, "epoch timestamp");
  const offset = BigInt(UTILITY_CANARY_SPEC.epochOffsetSeconds);
  check(value >= offset, "epoch timestamp predates Monday offset");
  return Number((value - offset) / BigInt(UTILITY_CANARY_SPEC.epochSeconds));
}

export function utilityProRataReward(weight, startIndex, endIndex) {
  ...
  return (w * (end - start)) / UTILITY_REWARD_Q128;
}
...
const checkTransaction = (record, expectedTarget, label) => {
  ...
  checkAddress(tx.from, UTILITY_CANARY_SPEC.deployer, `${label} sender`);
...
const checkApproval = (record, spender, expectedAmount, label) => {
  checkTransaction(record, UTILITY_CANARY_SPEC.hookrToken, label);
  ...
  checkAddress(record.event.owner, UTILITY_CANARY_SPEC.deployer, `${label} event owner`);
 ...
 */
export function validateUtilityCanaryEvidence(evidence) {
  const spec = UTILITY_CANARY_SPEC;
  ...
  check(evidence.receiptOrder.length === UTILITY_RECEIPT_LABELS.length, "receipt order is not exactly 8 records");
  for (let index = 0; index < UTILITY_RECEIPT_LABELS.length; index += 1) {
    check(evidence.receiptOrder[index]?.label === UTILITY_RECEIPT_LABELS[index], `receipt #${index} label is wrong`);
  }
  ...
  for (const label of UTILITY_RECEIPT_LABELS) {
  ...
  const depositEpoch = utilityEpochAt(lock.transaction.timestamp);
  ...
  check(
    asNumber(lock.event.expiryEpoch, "expiry epoch") === utilityEpochAt(lock.event.unlockAt),
  ...
  check(utilityEpochAt(boost.transaction.timestamp) === depositEpoch + 1, "boost was not mined in activation epoch");
  ...
  check(utilityEpochAt(claim.transaction.timestamp) === depositEpoch + 2, "claim was not mined in following epoch");
  ...
  check(
    asNumber(post.lockPosition.expiryEpoch, "post expiry epoch") === utilityEpochAt(lock.event.unlockAt),
  ...
  const expectedReward = utilityProRataReward(
