...
const fail = (message) => {
  console.error(`\nUTILITY DEPLOYMENT PREFLIGHT BLOCKED: ${message}`);
  process.exit(1);
};
...
const expectedShape = [
  { type: "CREATE", contractName: "HookrLockRewardsV1" },
  { type: "CREATE", contractName: "HookrLaunchBoostV1" },
...
try {
  validateReviewedRuntime({
    ...
    expectedImmutableAddresses: [HOOKR_TOKEN],
  ...
  validateReviewedRuntime({
    ...
    expectedImmutableAddresses: [HOOKR_TOKEN, launchpad, rewards],
...
const equalityChecks = [
  ...
  [await read(rewards, rewardsAbi, "contractVersion"), "1", "LockRewards version"],
  ...
  [await read(boost, boostAbi, "contractVersion"), "1", "LaunchBoost version"],
...
console.log("UTILITY DEPLOYMENT PREFLIGHT OK (read-only)");
...
console.log("  proof     3/3 canonical receipts; constructors, one-shot linkage, policy and ambient-safe accounting exact");
